...
const ServerPrototype = {
   ...
   * @param {String || Object} [opts] 参数，eg: { url: 自动刷新脚本的路径地址, type: 'polling || eventsource' }
   */
  livereload(opts) {
    ...
    opts = Object.assign({ timeout: 10000, type: 'polling' }, opts || {});

    let Reloader;
    if (opts.type === 'eventsource') {
      Reloader = require('./livereload/eventsource');
    } else {
      Reloader = require('./livereload/polling');
    }
    this._reloader = new Reloader(opts);
